...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
  FlutterMethodChannel* channel = [FlutterMethodChannel
      methodChannelWithName:@"com.baseflow.flutter/location_permissions"
    ...
    [LocationPermissionsPlugin checkServiceStatus:result];
      ...
      if (_result != nil) {
        result([FlutterError errorWithCode:@"ERROR_ALREADY_REQUESTING_PERMISSION"
                                   message:@"A request for permissions is already running, please wait for it to finish before doing another request."
                                   details:nil]);
        return;
      }
...
+ (void)checkServiceStatus:(FlutterResult)result {
    ServiceStatus serviceStatus = [CLLocationManager locationServicesEnabled] ? ServiceStatusEnabled : ServiceStatusDisabled;
...
+ (PermissionStatus)getPermissionStatus:(PermissionLevel)permissionLevel {
    ...
    PermissionStatus status = [LocationPermissionsPlugin  determinePermissionStatus:permissionLevel authorizationStatus:authorizationStatus];
...
- (void)requestPermission:(FlutterMethodCall*)call {
    ...
    if (level == PermissionLevelLocation) {
        if ([[NSBundle mainBundle] objectForInfoDictionaryKey:@"NSLocationAlwaysUsageDescription"] != nil) {
            ...
            _result([FlutterError errorWithCode:@"ERROR_MISSING_PROPERTYKEY"
                                        message:@"To use location in iOS8 you need to define either NSLocationWhenInUseUsageDescription or NSLocationAlwaysUsageDescription in the app bundle's Info.plist file"
                                        details:nil]);
            _result = nil;
            return;
        }
        ...
        if ([[NSBundle mainBundle] objectForInfoDictionaryKey:@"NSLocationAlwaysUsageDescription"] != nil) {
            ...
            _result([FlutterError errorWithCode:@"ERROR_MISSING_PROPERTYKEY"
                                        message:@"To use location in iOS8 you need to define NSLocationAlwaysUsageDescription in the app bundle's Info.plist file"
                                        details:nil]);
            _result = nil;
            return;
        }
        ...
        if ([[NSBundle mainBundle] objectForInfoDictionaryKey:@"NSLocationWhenInUseUsageDescription"] != nil) {
            ...
            _result([FlutterError errorWithCode:@"ERROR_MISSING_PROPERTYKEY"
                                        message:@"To use location in iOS8 you need to define NSLocationWhenInUseUsageDescription in the app bundle's Info.plist file"
                                        details:nil]);
            _result = nil;
            return;
        }
...
- (void)locationManager:(CLLocationManager *)manager didChangeAuthorizationStatus:(CLAuthorizationStatus)status {
    ...
    PermissionStatus permissionStatus = [LocationPermissionsPlugin
                                         determinePermissionStatus:_permissionLevel
...
+ (PermissionStatus)determinePermissionStatus:(PermissionLevel)permissionLevel authorizationStatus:(CLAuthorizationStatus)authorizationStatus {
